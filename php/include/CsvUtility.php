...
class CsvUtility
{
    ...
    public function getList($key, $value, $conditions = array())
    {
        $data = $this->find(array($key, $value), $conditions);
    ...
    public function fetch($header = true, $delimiter = ',')
    {
        return self::fetchCSV($this->file, $header, $delimiter);
    }
    ...
    public function write($data, $key_as_header = false, $delimeter = ',', $mode = "w")
    {
        return self::writeCSV($this->file, $data, $key_as_header, $delimeter, $mode);
    }
            ...
            else
            {
                ...
                if (isset($temp['header']) && !empty($temp['header']))
                {
                    ...
                    if ($print_header)
                    {
                        ...
                        $data = array_merge($data, isset($temp['data']) ? $temp['data'] : array());
                    }
                    else
                    {
                        $data = isset($temp['data']) ? $temp['data'] : array();
                    }
                }
                else
                {
                    $data = isset($temp['data']) ? $temp['data'] : $temp;
                }
            }
